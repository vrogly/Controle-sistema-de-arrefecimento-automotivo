...
dTi = [(mdot*cp*(T3-T1) + Qdotger +(mdot /rho)*Delta_P_H2O ) / (rho * V_motor * cp), %
    (-F*U_A*((T1-T2)/(log((T1-T_ar)/(T2-T_ar)))) + mdot*cp*(T1-T2) -(mdot/rho)*Delta_P_R ) / (rho * V_I * cp),%
    (-F*U_A*((T2-T3)/(log((T2-T_ar)/(T3-T_ar)))) + mdot*cp*(T2-T3) -(mdot/rho)*Delta_P_R ) / (rho * V_II * cp),
    ]
A = jacobian(dTi, [T1,T2,T3])
